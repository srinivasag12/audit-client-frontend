(function (){
    ...
    angular
		    .module('app.certificate.detailsihmExe')        
		    .controller('certificateHistoryControllerIhmExe', certificateHistoryControllerIhmExe)
		    .controller('CertificateDetailsControllerIhmExe', CertificateDetailsControllerIhmExe)
		    .controller('ihmCertificatesExeController', ihmCertificatesExeController)
		    .controller('completionSurveyDatePopUpController', completionSurveyDatePopUpController); 
    
    
    function CertificateDetailsControllerIhmExe(userFactory,masterFactory,ModalService,blockUI,AppConstant,auditType,$window,$state,$scope,certificateFactory,$cookies,$timeout,certDtlRequiredData,toaster,YYYYMMDD,MMMDDYYYY,DDMMYYYY,DDMMMYYYY,HHmm,detailsFactory,auditService,auditFactory,detailsFactoryIhm,CERTI_URL,vesselStatementFactory){
    	...
    	certDetails.onChangeReissue = false;
    ...
    detailsFactory.getCurrentUserDetail(sessionStorage.getItem('emailId'),sessionStorage.getItem('companyId')).$promise.then(function(res){ 
     ...
     $timeout(function(){
    	...
    	certDetails.CertTypeForIHM           = [certDetails.AppConstant.FULL_TERM_IHM,certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM];
    	...
    	if($state.params.certificate){
			if($state.params.certificate == 'Audit'){
				...
				if(auditFactory.getCertificateDetails()){
					...
					certDetails.certificateDtl = angular.copy(auditFactory.getCertificateDetails()).auditDetail;
						...
						if(auditAndCertData.auditDetail.certificateDetail){
							...
							certDetails.tempAuditData.certificateData = certDetails.certificateDtl.certificateData.filter(function(val) {
							    return val.socType == 'exe' ||   val.socType != "EXEMPTION";
							});
							
							certDetails.tempAuditData.certificateDataHistory = certDetails.certificateDtl.certificateDataHistory.filter(function(val) {
							    return val.socType == 'exe' ||  val.socType != "EXEMPTION";
							});
							if(certDetails.tempAuditData.certificateDetail){
								certDetails.tempAuditData.certificateDetail = certDetails.tempAuditData.certificateDetail.filter(function(val) {
							    return val.socType == 'exe' ||   val.socType != "EXEMPTION" ;
							});
				...
				//	certDetails.signerName = certDetails.certificateDtl.leadAuditorName ? certDetails.certificateDtl.leadAuditorName : auditAndCertData.auditDetail.leadAuditorName;
					...
					certDetails.certificateDtl.certIssueId = certDetails.certificateDtl.certificateData.length > 0 ? certDetails.certificateDtl.certificateData[0].certIssueId : certDetails.AppConstant.FULL_TERM_IHM;
...
//					
			...
			//		certDetails.certificateDtl.issuerName = certDetails.certificateDtl.issuerName ? certDetails.certificateDtl.issuerName : (auditAndCertData.auditDetail.leadAuditorName ? auditAndCertData.auditDetail.leadAuditorName : certDtlRequiredData.getCurrentUserFullName.userName);
					...
					});   //changed by sudharsan for jira-ID = IRI-5566
				}
...
//    			setSocTypeCheckBoxValue(certDetails.certificateDtl.certIssueId,certDetails.certificateDtl.companyId,certDetails.certificateDtl.vesselImoNo,certDetails.certificateDtl.auditSubTypeId,certDetails.certificateDtl.auditSeqNo);
    	...
    	function IHMDisableFunc(){
    		var v = ((
    				((certDetails.certificateDtl.certificateDetail) || certDetails.IhmDisable ) && (certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.IHM_INITIAL_AUD_SUBTYPE_ID || certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.IHM_AMENDMENT_AUD_SUBTYPE_ID) && !certDetails.withOutAdt && certDetails.IhmDisable)
    				|| certDetails.disableAll  || certDetails.releaseLock) 
    				|| certDetails.socLength == 0;
    			return v;
    	}
...
//    		certDetails.certificateDtl.issuerName = certDetails.signerName;
	...
	//		 certDetails.certificateDtl.certIssueDate = moment(certDetails.certificateDtl.auditDate,YYYYMMDD).format(MMMDDYYYY);
    	...
    	function getCertificateDetails(){
    		...
    		certificateFactory.getCertificateDetailForIhm(certDetails.auditSeqNo, certDetails.companyId, certDetails.seqNo,certDetails.activeStatus,'exe').$promise.then(function(res){
...
//      				getCertificateNoWithOutAudit();
    	...
    	function getAllIhmCertifiactes(){
				...
				certificateFactory.getAllCertificateDetailForIhm(certDetails.certificateDtl.auditTypeId, certDetails.certificateDtl.vesselImoNo, certDetails.certificateDtl.companyId,'exe').$promise.then(function(res){
...
//	
    	...
    	function setSocTypeCheckBoxValue(certIssueId,companyId,vesselImoNo,auditSubTypeId,auditSeqNo,certLink){
    		...
    		certificateFactory.getSocTypeSelectDefault(certIssueId,companyId,vesselImoNo,auditSubTypeId,auditSeqNo,certLink,'exe').$promise.then(function(res){
    	...
    	function viewCertificate(){
    		...
    		ModalService.showModal({
    			...
    			controller  : 'certificateHistoryControllerIhmExe as certHist',
...
//    		certData.auditDate = certDetails.certificateDtl.auditDate;
    	...
    	function onChangeCompletionDate(){
    		...
    		certDetails.certificateDtl.certExpireDate =  certDetails.certificateDtl.completionSurveyDate? moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY):'';
    		...
    		certDetails.minCertExpireDate = moment(certDetails.certificateDtl.completionSurveyDate, MMMDDYYYY).format(MMMDDYYYY);
    			...
    			certDetails.certificateDtl.extendedExpireDate =  certDetails.certificateDtl.completionSurveyDate? moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY):'';
    				 ...
    				 detailsFactory.vesselDetails(certDetails.certificateDtl.companyId,sessionStorage.getItem('emailId'),certDetails.certificateDtl.vesselImoNo,'vesselImoNo').$promise.then(function(res){
            			...
            			vesselStatementFactory.getVesselDetails(certDetails.certificateDtl.vesselImoNo,certDetails.certificateDtl.companyId).$promise.then(function(vesRes){
            				...
            				certDetails.certificateDtl.vesselCompanyAddress =vesRes.vsselDtl[0].registeredCompanyAddress;
    		   	       		...
    		   	       		certDetails.certificateDtl.keelLaidDate = vesRes.vsselDtl[0].keelLaidDate && vesRes.vsselDtl[0].keelLaidDate != '' && vesRes.vsselDtl[0].keelLaidDate != null ? dateFormater(vesRes.vsselDtl[0].keelLaidDate , 'DD-MMM-YYYY') : '';
            			 ...
            			 if( (certDetails.tempAuditData.auditStatusId == 1002 && (!certDetails.certificateDtl.publishStatus || certDetails.certificateDtl.publishStatus == 0 ))){
            				...
            				certDetails.certificateDtl.vesselCompanyAddress =res[0].registeredCompanyAddress;
    		   	       		...
    		   	       		certDetails.certificateDtl.keelLaidDate = res[0].keelLaidDate && res[0].keelLaidDate != '' && res[0].keelLaidDate != null ? dateFormater(res[0].keelLaidDate , 'DD-MMM-YYYY') : '';
            				...
            				if(res[0].registeredCompanyAddress ===certDetails.certificateDtl.vesselCompanyAddress)
            					...
            					console.log(res[0].registeredCompanyAddress + "   "+certDetails.certificateDtl.vesselCompanyAddress);
           	       			...
           	       			if(res[0].registeredCompanyName == certDetails.certificateDtl.vesselCompanyName)
        	   	       		 ...
        	   	       		 if((res[0].keelLaidDate != '' || res[0].keelLaidDate != null ) || dateFormater(res[0].keelLaidDate,'DD-MMM-YYYY',true) == dateFormater(certDetails.certificateDtl.keelLaidDate,'DD-MMM-YYYY',true))
        	   	       		...
        	   	       		if(count!=9){
        	   	       			 ...
        	   	       			 if(certDetails.certificateDtl.certificateDetail && certDetails.certificateDtl.certIssueId != 1008 && certDetails.certificateDtl.auditSubTypeId != certDetails.AppConstant.INITIAL_SUB_TYPE_ID && certDetails.certificateDtl.auditSubTypeId != certDetails.AppConstant.INTERIM_SUB_TYPE_ID){
		    		    	   	       	...
		    		    	   	       	if(certDetails.certificateDtl.certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM){
			        		   	       		...
			        		   	       		certDetails.certificateDtl.keelLaidDate = certDetails.latestVesselDetail[0].keelLaidDate &&  certDetails.latestVesselDetail[0].keelLaidDate != '' && certDetails.latestVesselDetail[0].keelLaidDate != null ? dateFormater(certDetails.latestVesselDetail[0].keelLaidDate , 'DD-MMM-YYYY') : '';
    	...
    	function ihmCertificatesPopup(){ 
    			...
    			if(certDetails.certificateDtl.certIssueId == certDetails.AppConstant.RE_ISSUE){
    				vesselSpecificDtl_Generate(certDetails.latestVesselDetail[0]);
    					...
    					certificateFactory.deleteCertificateForIhm(certDetails.certificateDtl.certIssueId,certDetails.certificateDtl.companyId,certDetails.certificateDtl.vesselImoNo,certDetails.certificateDtl.auditSubTypeId,auditSeqNo,certNo,'exe').$promise.then(function(res){
    					 var resData = [];
    					 var resultData = [];
    					 var len = 1;
    					 var certificateData = [];
    						...
    						certDetails.certificateDtl.socType = 'exe';
    						var certificateData1 =  certDetails.setCertDetails();
    						 certificateData.push(certificateData1);
    					
    					// for Full term certificates
    				if(certDetails.certificateDtl.certIssueId == certDetails.AppConstant.FULL_TERM_IHM || certDetails.certificateDtl.certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM){
    			...
    			var validatIssueDate='';
    			 ...
    			 if(certDetails.certificateDtl.certIssueDate){
    				 ...
    				 if(certDetails.certificateDtl.auditTypeId != certDetails.AppConstant.IHM_TYPE_ID){
	    				 if(certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.INTERIM_SUB_TYPE_ID ){
	    					...
	    					expiryDate = moment(moment(certDetails.certificateDtl.certIssueDate, MMMDDYYYY).add(6,'months'),DDMMYYYY).subtract(1,'days').format(MMMDDYYYY);
	    					 ...
	    					 expiryDate=moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
	    					
	    					 issueDate=moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
	    					
	    					
	    					 if(certDetails.withOutAdt){
	    						 certDetails.certificateDtl.certExpireDate=moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
	    	 	     				}
    					  ...
    					  if(certDetails.certificateDtl.auditTypeId == certDetails.AppConstant.IHM_TYPE_ID){
    						if(certDetails.isIssuExpIhmChange){
    						  ...
    						  certDetails.certificateDtl.certIssueDate = certDetails.certificateDtl.certIssueDate ? certDetails.certificateDtl.certIssueDate : certDetails.certificateDtl.auditDate;
    							...
    							certDetails.certificateDtl.certExpireDate = certDetails.certificateDtl.certExpireDate != '' ? certDetails.certificateDtl.certExpireDate : moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
    							certDetails.isIssuExpIhmChange = false;
    						}
    					 }else{
    						 certDetails.certificateDtl.certExpireDate = certDetails.certificateDtl.certIssueDate ? moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY) : certDetails.certificateDtl.certExpireDate;
    					 }
    					  
    					 if(certDetails.withOutAdt){
    						 if(certDetails.certificateDtl.auditTypeId == certDetails.AppConstant.IHM_TYPE_ID){
    							 certDetails.certificateDtl.certExpireDate = certDetails.orgCertificateDtl.certIssueDate ?   moment(certDetails.orgCertificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY) : moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY) ;
    						 }else{
    							 certDetails.certificateDtl.certExpireDate=moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
    						 }
    					 }
    				 ...
    				 if(certDetails.certificateDtl.publishStatus==0 && certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.RENEWAL_SUB_TYPE_ID || certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.INITIAL_SUB_TYPE_ID ){
     					certDetails.maxCertExpireDate=moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
    		 ...
    		 function setExpireDateIHM(){
    			...
    			var validatIssueDate='';
    			 ...
    			 if(certDetails.certificateDtl.certIssueDate){
    				 ...
    				 validatExpireDate  =  moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
    				 
    				 certDetails.minCertIssueDate      = '';
    				 
    				 certDetails.maxCertIssueDate      = '';
    				 certDetails.minCertExpireDate     = moment(certDetails.certificateDtl.completionSurveyDate, MMMDDYYYY).format(MMMDDYYYY);
    				 certDetails.maxCertExpireDate     = moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
    				 ...
    				 if(certDetails.certificateDtl.extendedIssueDate){
    					 ...
    					 if(certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.IHM_INITIAL_AUD_SUBTYPE_ID){
    						...
    						// certDetails.minExtnCertIssueDate=moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).format(YYYYMMDD);
    						 ...
    						 issueDate=moment(certDetails.certificateDtl.extendedIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
    						 
    						 expiryDate=moment(certDetails.certificateDtl.extendedIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
    					 ...
    					 certDetails.maxExtnCertExpireDate = moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
//    					 if(certDetails.certificateDtl.publishStatus==0 && certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.RENEWAL_SUB_TYPE_ID || certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.INITIAL_SUB_TYPE_ID ){
//    	     					certDetails.maxExtnCertExpireDate=moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
//    	     					
//    	     				}
		...
		if((certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.INITIAL_SUB_TYPE_ID && certDetails.certificateDtl.certIssueId == certDetails.AppConstant.FULL_TERM_IHM && certDetails.certificateDtl.auditTypeId!=certDetails.AppConstant.IHM_TYPE_ID) ||
			(certDetails.certificateDtl.auditTypeId==certDetails.AppConstant.IHM_TYPE_ID && certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.IHM_INITIAL_AUD_SUBTYPE_ID && 
			 certDetails.certificateDtl.certIssueId == certDetails.AppConstant.FULL_TERM_IHM  )){
    			 if(moment(certDetails.certificateDtl.certExpireDate,MMMDDYYYY).subtract(3,'years').add(1,'days').format(YYYYMMDD)>moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).format(YYYYMMDD)){
    					toaster.info('Validity of the certificate is more than 3 Years');
    					certDetails.validateFlagForIssueExpDate = false;
    					certDetails.disableGenerate = true;
       			 }else if(moment(certDetails.certificateDtl.certExpireDate,MMMDDYYYY).subtract(3,'years').add(1,'days').format(YYYYMMDD)<moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).format(YYYYMMDD)){
       				    toaster.info('Validity of the certificate is less than 3 Years');
			...
			if(moment(certDetails.certificateDtl.certExpireDate,MMMDDYYYY).subtract(6,'months').add(1,'days').format(YYYYMMDD)>moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).format(YYYYMMDD)){
				...
				(certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.IHM_INITIAL_AUD_SUBTYPE_ID && certDetails.certificateDtl.certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM && certDetails.certificateDtl.auditTypeId==certDetails.AppConstant.IHM_TYPE_ID)){
				 if(moment(certDetails.certificateDtl.extendedExpireDate,MMMDDYYYY).subtract(3,'years').add(1,'days').format(YYYYMMDD)>moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).format(YYYYMMDD)){
					toaster.info('Validity of the certificate is more than 3 Years');
					certDetails.validateFlagForIssueExpDate = false;
					certDetails.disableGenerate = true;
	   			 }else if(moment(certDetails.certificateDtl.extendedExpireDate,MMMDDYYYY).subtract(3,'years').add(1,'days').format(YYYYMMDD)<moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).format(YYYYMMDD)){
	   				toaster.info('Validity of the certificate is less than 3 Years');
    		 ...
    		 function getAuditCertDetails(){
    			 ...
    			 certificateFactory.getAuditCertDetailsForIhm(certDetails.auditSeqNo, certDetails.companyId).$promise.then(function(res){
    				...
    				// this is used to get the prevoius certificate
					 ...
					 certDetails.socLength = certDetails.tempAuditData.certificateDetail.filter(function(val) {
					    return val.socType == 'exe'  &&  val.socType != "EXEMPTION"; ;
					}).length;
  			...
  			//		certDetails.certificateDtl.issuerName = certDetails.signerName;
    			...
    			if(res.initalCount==1 || res.renewalCount==1){
					...
					certDetails.certificateNotPublish = res.certificateNotPublish ? res.certificateNotPublish : false;
    				 ...
    				 certDetails.leadStatus = (detailsLead.userId == sessionStorage.getItem('emailId') || sessionStorage.getItem('userRoleId') ==  certDetails.AppConstant.IHM_MANAGER_ROLE_ID) ? true : false;
...
//    				 }
    				 ...
    				 if(certDetails.leadStatus &&  (auditAndCertData.auditDetail.certificateDetail.length > 0 && certDetails.socLength > 0)){
    					// certDetails.certificateDtl = angular.copy(auditAndCertData.auditDetail.certificateDetail[0]);
    					 certDetails.certificateDtl = angular.copy(angular.copy(auditAndCertData.auditDetail.certificateDetail[0].socType == 'exe' ? auditAndCertData.auditDetail.certificateDetail[0] : certDetails.certificateDtl));
                getUserDet(certDetails.certificateDtl.issuerId,certDetails.certificateDtl.companyId);
    					 certDetails.certificateDtl.issuerName = certDetails.certificateDtl.issuerName;
    					 if(auditAndCertData.auditDetail.certificateDetail.length > 1){
    					 if(auditAndCertData.auditDetail.certificateDetail[1].auditDate !=  moment(certDetails.auditDtlsAuditDate,MMMDDYYYY).format(YYYYMMDD) ){
    						 certDetails.certificateDtl.auditDate = certDetails.auditDtlsAuditDate != '' && certDetails.auditDtlsAuditDate ? moment(certDetails.auditDtlsAuditDate,MMMDDYYYY).format(YYYYMMDD) :certDetails.certificateDtl.auditDate;
    					 }
    				 ...
    				 if(!certDetails.leadStatus){
    					 certDetails.certificateDtl = angular.copy(angular.copy(auditAndCertData.auditDetail.certificateDetail.length > 0 && auditAndCertData.auditDetail.certificateDetail[0].socType == 'exe' ? auditAndCertData.auditDetail.certificateDetail[0] : certDetails.certificateDtl));
    					  getUserDet(certDetails.certificateDtl.issuerId,certDetails.certificateDtl.companyId);
    				 ...
    				 if((certDetails.userRoleId == certDetails.AppConstant.ADMIN_ROLE_ID || certDetails.userRoleId == certDetails.AppConstant.IHM_MANAGER_ROLE_ID) && auditAndCertData.auditDetail.makeFinal == 0){
 						certDetails.disableAll = true;
    				 ...
    				 certDetails.enableCertType.push(certDetails.certificateDtl.certIssueId);
    				 certDetails.enableCertTypeForIHM.push(certDetails.certificateDtl.certIssueId);
    				 if(auditAndCertData.auditDetail.certificateDetail.length >1 && certDetails.socLength > 0){
    	    		...
    	    		if(certDetails.userRoleId == certDetails.AppConstant.IHM_MANAGER_ROLE_ID){
    	    			if(certDetails.certificateDtl.publishStatus == 1){
    	    				...
    	    				if(certData.length > 0){
    	    					if(certDetails.tempAuditData.lockHolder == certDetails.loginUserId && certDetails.userRoleId != certDetails.AppConstant.IHM_MANAGER_ROLE_ID.toString()|| certDetails.certificateDtl.issuerId == certDetails.loginUserId ){
    	    	    				certDetails.disableLock = false;
    	    	    				}
    	    	    				else {
    	    	...
    	    	//	certDetails.disableLock = auditAndCertData.auditDetail.lockHolder && (auditAndCertData.auditDetail.lockHolder != certDetails.loginUserId)  ? true : false;
    	    		//certDetails.disableLock = (certDetails.certificateDtl.leadAuditorId && (certDetails.certificateDtl.leadAuditorId == certDetails.loginUserId)) ? false : true;
    	    				...
    	    				if(certDetails.tempAuditData.leadSign){
    	    					...
    	    					if(isNaN(parseInt(certDetails.tempAuditData.leadAuditorId))){
    	    						certDetails.disableAll = false;
    	    					}
    	    					else {
    	    						if(!(certDetails.tempAuditData.userIns) ||certDetails.tempAuditData.userIns == certDetails.loginUserId){
    	    							certDetails.disableLock = false;
    	    						}
    	    						else{
    	    						certDetails.disableLock = true;
...
//    				var doc = new jsPDF('p', 'mm', 'a4');	
    	...
    	function setDateAndDisableCertIssueExpiryIHM(){ 
    		...
    		if(certDetails.certificateDtl.auditTypeId == certDetails.AppConstant.IHM_TYPE_ID ){
    				...
    				certificateFactory.getAllCertificateDetailForIhm(certDetails.certificateDtl.auditTypeId, vesselImoNo, certDetails.certificateDtl.companyId,'exe').$promise.then(function(res){
    					...
    					if(auditAndCertData && auditAndCertData.addSurvDueDateChanged){
     				  ...
     				  if(certDetails.AllIhmCertificateData.length > 0 && (certDetails.AllIhmCertificateData[0].certIssueId == certDetails.AppConstant.EXTENSION_IHM || certDetails.AllIhmCertificateData[0].certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM)){
    					  ...
    					  if(certDetails.certificateDtl.certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM){
       	       				...
       	       				certDetails.maxExtnCertExpireDate =  moment(certDetails.certificateDtl.completionSurveyDate, MMMDDYYYY).add(3,'years').subtract(1,'days').format(YYYYMMDD);
       	    		}
      					if(certDetails.certificateDtl.certIssueId ==  certDetails.AppConstant.ADDITIONAL_ENDORSED_IHM && certDetails.certificateDtl.auditSubTypeId ==  certDetails.AppConstant.IHM_AMENDMENT_AUD_SUBTYPE_ID){
    	...
    	function onChangeIssueType(){
    		...
    		if(certDetails.certificateDtl.certIssueId ==  certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM || certDetails.certificateDtl.certIssueId ==  certDetails.AppConstant.EXTENSION_IHM ){
    			...
    			//certDetails.certificateDtl.dateOfRegistry = certDetails.orgCertificateDtl.dateOfRegistry;
    		//	certDetails.certificateDtl.grt = certDetails.orgCertificateDtl.grt;
    					  ...
    					  console.log(certDetails.latestVesselDetail)
    	 	    			console.log(certDetails.enableCertType);
    					  	...
    					  	certDetails.certificateDtl.vesselCompanyName = certDetails.latestVesselDetail[0].registeredCompanyName;
    		   	       		...
    		   	       		certDetails.certificateDtl.keelLaidDate = certDetails.latestVesselDetail[0].keelLaidDate && certDetails.latestVesselDetail[0].keelLaidDate != '' && certDetails.latestVesselDetail[0].keelLaidDate != null ?  moment(certDetails.latestVesselDetail[0].keelLaidDate,'YYYY-MM-DD').format('DD-MMM-YYYY'): '';
    			...
    			if(certDetails.certificateDtl.certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM){
    				...
    				certDetails.certificateDtl.extendedExpireDate = moment(certDetails.certificateDtl.auditDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY)
    			}
    		}
    		if(certDetails.certificateDtl.certIssueId != certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM){
					  	...
					  	certDetails.certificateDtl.vesselCompanyName = certDetails.orgRegCompanyName;
		   	       		...
		   	       		certDetails.certificateDtl.keelLaidDate = certDetails.orgCertKeelLaidDate && certDetails.orgCertKeelLaidDate != '' && certDetails.orgCertKeelLaidDate != null ? dateFormater(certDetails.orgCertKeelLaidDate,'DD-MMM-YYYY'):'';
    		...
    		if(certDetails.certificateDtl.certIssueId ==  certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM){
    			...
    			certDetails.onChangeReissue = true;
    		}
    		else{
    			certDetails.disableEcField = false;
    		}
    	...
    	function setCertIssueExpiry(){
           	...
           	console.log(certDetails.orgCertificateDtl);
        		certDetails.getPreviousAuditCerData = angular.copy(auditFactory.getCertificateDetails()).previousAudit;
            		...
            		certDetails.certificateDtl.certReissueReason=1010;
            		//certDetails.setExtendedExpireDate();
    	...
    	function renewalFulltermCertDtls(){
    		...
    		certDetails.certificateDtl.certExpireDate = certDetails.certificateDtl.certExpireDate?certDetails.certificateDtl.certExpireDate:'';
    				//moment(certDetails.certificateDtl.certIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY):'';
...
certDetails.checkCertVal = function(certValue,certIssueId){
		  ...
		  if(certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.IHM_INITIAL_AUD_SUBTYPE_ID){
			  ...
			  if(certDetails.auditDetailIhm){
				  ...
				  certDetails.certificateDtl.extendedIssueDate = certDetails.auditDetailIhm.auditDate ? moment(certDetails.auditDetailIhm.auditDate, YYYYMMDD).format(MMMDDYYYY)  : moment(certDetails.auditDetailIhm.certIssueDate, YYYYMMDD).format(MMMDDYYYY) ;
   				  certDetails.certificateDtl.extendedExpireDate = moment(certDetails.certificateDtl.extendedIssueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
				  ...
				  certDetails.certificateDtl.extendedIssueDate =  moment(new Date()).format(DDMMMYYYY);// certDetails.orgCertificateDtl.certIssueDate ? moment(certDetails.orgCertificateDtl.certIssueDate, MMMDDYYYY).format(MMMDDYYYY)  : '' ;
   				  certDetails.certificateDtl.extendedExpireDate = moment(certDetails.certificateDtl.completionSurveyDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
			...
			//  certDetails.certificateDtl.completionSurveyDate =certDetails.certificateDtl.certIssueDate;
			  certDetails.certificateDtl.extendedExpireDate =  moment(certDetails.certificateDtl.completionSurveyDate, MMMDDYYYY).add(3,'years').subtract(1,'days').format(MMMDDYYYY);//moment(certDetails.certificateDtl.extendedIssueDate).add(3,'years').subtract(1,'days').format(MMMDDYYYY);
  ...
  if(certDetails.certificateDtl.certIssueId ==  certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM){
	...
	if(certDetails.certificateDtl.auditSeqNo == certDetails.AppConstant.IHM_NO_AUD_CERT_AUDITSEQ){
		...
		/**End Here */ 
     	...
     	function auditPlaceDisable(){
    	  ...
    	  function checkMinandMaxRenewalDates(auditDate,expDate2,nextrenewalStartDate){
    		  ...
    		  if(auditDate < nextrenewalStartDate){
    			  ...
    			  certDetails.maxCertIssueDate= moment(certDetails.certificateDtl.creditDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(YYYYMMDD); 
        		  ...
        		  certDetails.maxCertExpireDate     = moment(issueDate,MMMDDYYYY).add(3,'years').subtract(1,'days').format(YYYYMMDD);
    	  ...
    	  function checkMinandMaxRenewalDatesReissue(auditDate,expDate2,nextrenewalStartDate){
    		...
    		//	     certDetails.minExtnCertIssueDate=moment(certDetails.certificateDtl.auditDate,MMMDDYYYY).format(YYYYMMDD);
					 certDetails.maxExtnCertIssueDate=moment(certDetails.certificateDtl.auditDate, MMMDDYYYY).add(3,'years').subtract(1,'days').format(YYYYMMDD);
     	...
     	function generateDisableIhm(){
			/**Added by sudharsan for JIRA-ID=IRI-5435 */
			if(certDetails.certificateDtl.auditStatusId == certDetails.AppConstant.COMPLETED_AUDIT_STATUS){
				certDetails.IhmFieldsDisable = true;
			}
			//End here JIRA-ID=IRI-5435
			 var res =  (!certDetails.validateFlagForIssueExpDate && 
						 ...
						 ||  certDetails.certificateDtl.auditStatusId == certDetails.AppConstant.COMPLETED_AUDIT_STATUS ; //Added by sudharsan for JIRA-ID=IRI-5435
			 return res;
		 }
     	...
     	function publishDisableIhm(){
     		var condition = (certDetails.certificateDtl.auditStatusId == certDetails.AppConstant.REOPEN)
							 ...
							 || (certDetails.certificateDtl.auditTypeId == certDetails.AppConstant.IHM_TYPE_ID 
							 && certDetails.certificateDtl.auditSubTypeId == certDetails.AppConstant.IHM_INITIAL_AUD_SUBTYPE_ID 
							 && certDetails.certificateDtl.certIssueId == certDetails.AppConstant.ADDITIONAL_ENDORSED_IHM)
							 certDetails.certificateDtl.auditStatusId == certDetails.AppConstant.COMPLETED_AUDIT_STATUS;  //Added by sudharsan for JIRA-ID=IRI-5435
     	 ...
     	 function disableOptionsIhm(data){
     		 if( ((data.certificateDetail[0].auditTypeId == certDetails.AppConstant.IHM_TYPE_ID && data.certificateDetail[0].certIssueId != certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM) || (data.certificateDetail[0].certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM && data.leadSign))){	
     			 ...
     			 auditFactory.getAllIhmAuditDetail(data.auditTypeId,data.vesselImoNo,data.companyId).$promise.then(function(res){
     				...
     				var socCertificates = certDetails.AllIhmAuditDetails[certDetails.AllIhmAuditDetails.length - 1].certificateDetail.filter(function(val) {
					    return (val.socType === 'exe');
					});
     				...
     				certDetails.isAmendCreated = true;
     				 ...
     				 if(res.length > 1 && res[res.length - 1].auditSummaryId != 1005 && res[res.length - 1].auditStatusId == 1001 && res[0].auditStatusId != 1004 && ( res[0].auditSummaryId != 1005 && !res[0].leadSign)){
     					 certDetails.disableAll = true;
     				 }
     				
     				
     				 else if(res.length > 1 && res[res.length - 1].certificateDetail.length > 0 && socCertificates.length > 1 && socCertificates[0].activeStatus == 1 && ((socCertificates[0].certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM && certDetails.certificateDtl.certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM) ||(socCertificates[0].certIssueId == certDetails.AppConstant.ADDITIONAL_ENDORSED_IHM) )){
     				...
     				if(socCertificates.length > 0 && (socCertificates[0].activeStatus == 1 && socCertificates[0].publishStatus==0 )){
     					certDetails.IhmDisable = true;
     				}
     			 ...
     			 if(data.auditSubTypeId == certDetails.AppConstant.IHM_INITIAL_AUD_SUBTYPE_ID)
     			 {
     				 ...
     				 if(certDetails.CertTypeForIHM.indexOf(certDetails.AppConstant.EXTENSION_IHM) === -1) certDetails.CertTypeForIHM.push(certDetails.AppConstant.EXTENSION_IHM);
     				 if(certDetails.CertTypeForIHM.indexOf(certDetails.AppConstant.RENEWAL_ENDORSED_11_7_IHM) === -1) certDetails.CertTypeForIHM.push(certDetails.AppConstant.RENEWAL_ENDORSED_11_7_IHM);
     				 if(certDetails.CertTypeForIHM.indexOf(certDetails.AppConstant.RENEWAL_ENDORSED_11_8_11_9_IHM) === -1) certDetails.CertTypeForIHM.push(certDetails.AppConstant.RENEWAL_ENDORSED_11_8_11_9_IHM);
     				 if( res.length > 1 && socCertificates.length > 1 && socCertificates[0].activeStatus == 1 && res[res.length - 1].certificateDetail[1].auditStatusId != 1004){
     				//	 certDetails.CertTypeForIHM.push(certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM);
     					...
     					if($state.params.certificate != 'Audit' && $state.params.certificate !=null){
     					 ...
     					 if(socCertificates.length > 1 && socCertificates[0].auditStatusId == 1002  && socCertificates[0].certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM && certDetails.certificateDtl.certIssueId == certDetails.AppConstant.RE_ISSUE_ADMINISTRATIVE_IHM && certDetails.certificateDtl.publishStauts == 1){
	...
	function disableIhmAmendment(data){
    		...
    		auditFactory.getAllIhmAuditDetail(certDetails.certificateDtl.auditTypeId,certDetails.certificateDtl.vesselImoNo,certDetails.certificateDtl.companyId).$promise.then(function(res){
    			...
    			else if (certDetails.certificateDtl.auditStatusId == 1002 && res.length>0 && res[0].auditSeqNo != certDetails.certificateDtl.auditSeqNo){
    				certDetails.disableAll = true;
    			}
 ...
 function completionSurveyDatePopUpController(scope,$scope,close,toaster,AppConstant,certificateFactory,data){
    	...
    	completionDatePopUp.close = function(result) { 
    		...
    		if(result == 'NO'){
    			...
    			completionDatePopUp.certDetails.certExpireDate = completionDatePopUp.certDetails.completionSurveyDate ? moment(completionDatePopUp.certDetails.completionSurveyDate,'DD-MMM-YYYY').add(3,'years').subtract(1,'days').format('DD-MMM-YYYY'):'';
    			close(result, 0);
 ...
 function ihmCertificatesExeController(scope,$scope,close,toaster,AppConstant,certificateFactory,data,data1){
    	...
    	ihmCertificates.disableOptionHk = true;
    	ihmCertificates.disableOptionEu = true;
    	ihmCertificates.disableOptionExe = false;
    ...
    function certificateHistoryControllerIhmExe(scope,$scope,auditFactory,detailsFactory,CERTI_URL,blockUI,toaster,AppConstant,MMMDDYYYY,YYYYMMDD,auditService,$timeout,$sce){
    	var certHist = this;
    	$scope.qrString = "";
    	certHist.qrCodeData ='';
